...
enum Commands {
    ...
    /// Descargar e instalar SideStore
    InstallSidestore {
        #[arg(short, long, env = "ILOADER_APPLE_ID")]   apple_id: String,
        #[arg(short, long, env = "ILOADER_APPLE_PASSWORD")] password: String,
        #[arg(short, long)] udid: Option<String>,
    ...
    /// Ver certificados de desarrollo
    Certs {
        #[arg(short, long, env = "ILOADER_APPLE_ID")]   apple_id: String,
        #[arg(short, long, env = "ILOADER_APPLE_PASSWORD")] password: String,
    },
...
async fn cmd_devices() -> Result<(), CliError> {
    ...
    for dev in &devs {
        let conn_type = match dev.connection_type {
            Connection::Usb => "USB", Connection::Network(_) => "Red", Connection::Unknown(_) => "?",
        };
        // FIX: to_provider toma 2 args: (addr, label: impl Into<String>)
        let provider = dev.to_provider(addr.clone(), "iloader");
...
async fn cmd_install(apple_id: &str, password: &str, udid: Option<&str>,
    ipa: &str, anisette_url: &str, data_dir: &PathBuf) -> Result<(), CliError>
{
...
async fn cmd_install_sidestore(apple_id: &str, password: &str, udid: Option<&str>,
    nightly: bool, live_container: bool, anisette_url: &str, data_dir: &PathBuf) -> Result<(), CliError>
{
    ...
    println!("🔗 Colocando pairing file…");
    match place_pairing_auto(&dev_udid, app_key).await {
        Ok(_)  => println!("✅ Pairing colocado"),
        Err(e) => { println!("⚠️  Pairing falló: {e}\n   Ejecuta después: iloader pairing --app {app_key}"); }
    }
    println!("\n🎉 Listo.\n   Ajustes → General → VPN y gestión de dispositivos → Confiar");
    Ok(())
}
...
async fn cmd_certs(apple_id: &str, password: &str, anisette_url: &str, data_dir: &PathBuf) -> Result<(), CliError> {
    ...
    if certs.is_empty() { println!("No hay certificados activos."); return Ok(()); }
...
async fn cmd_revoke_cert(apple_id: &str, password: &str, serial: &str,
    anisette_url: &str, data_dir: &PathBuf) -> Result<(), CliError>
{
    let mut sideloader = authenticate(apple_id, password, anisette_url, data_dir).await?;
    let team = sideloader.get_team().await.map_err(|e| CliError::Auth(e.to_string()))?;
...
async fn cmd_app_ids(apple_id: &str, password: &str, anisette_url: &str, data_dir: &PathBuf) -> Result<(), CliError> {
    ...
    let resp = sideloader.get_dev_session()
        ...
        .map_err(|e| CliError::Auth(e.to_string()))?;
    // max_quantity es Option<u64>, available_quantity es Option<i64>
    println!("App IDs ({} registrados, max: {}, disponibles: {}):\n",
        resp.app_ids.len(),
        resp.max_quantity.map_or("?".to_string(), |v| v.to_string()),
        resp.available_quantity.map_or("?".to_string(), |v| v.to_string()));
    for app in &resp.app_ids {
        // FIX: app.name es String, no Option<String> → no usar as_deref()
...
async fn authenticate(email: &str, password: &str, anisette_url: &str, data_dir: &PathBuf)
...
async fn select_device(udid: Option<&str>)
    -> Result<(idevice::provider::UsbmuxdProvider, String), CliError>
{
    let mut conn = usbmuxd_conn().await?;
    let devs = conn.get_devices().await.map_err(|e| CliError::Usbmuxd(e.to_string()))?;
    ...
    // FIX: to_provider(addr, label) — 2 argumentos, label es el segundo
    let provider = dev.to_provider(usbmuxd_addr()?, "iloader");
    Ok((provider, udid_str))
}

async fn lockdown_info(provider: &idevice::provider::UsbmuxdProvider) -> (String, String) {
    let Ok(mut lc) = LockdownClient::connect(provider).await else {
        return ("Desconocido".into(), "?".into());
    };
    // FIX: get_value(domain: Option<&str>, key: Option<&str>) — 2 argumentos
...
async fn generate_pairing_bytes(udid: &str) -> Result<Vec<u8>, CliError> {
    ...
    // FIX: to_provider(addr, label) — 2 argumentos
    let provider = dev.to_provider(usbmuxd_addr()?, "iloader");
    ...
    let _ = lc.set_value(
        "EnableWifiDebugging", true.into(),
        Some("com.apple.mobile.wireless_lockdown"),
    ).await;

    // FIX: get_value(domain, key) — 2 argumentos
    ...
    if is_ios_below(&ver_str, 17, 4) {
        ...
        let dict = lockdown_plist.as_dictionary()
            .ok_or_else(|| CliError::Pairing("Lockdown plist no es diccionario".into()))?;
        let mut out = Vec::new();
    ...
    let rp_bytes = generate_rppairing(&provider, "iloader").await
        .map_err(|e| CliError::Pairing(e.to_string()))?.to_bytes();
...
async fn generate_rppairing(provider: &impl IdeviceProvider, hostname: &str)
    -> Result<RpPairingFile, idevice::IdeviceError>
{
    ...
    // create_software_tunnel(self) consume proxy → Adapter
    // to_async_handle() → AdapterHandle (tiene connect() público)
    let adapter = proxy.create_software_tunnel()?;
    let mut handle = adapter.to_async_handle();

    info!("Performing RSD handshake on port {rsd_port}…");
    ...
    let tunnel_port = tunnel_svc.port;

    let mut pairing_file = RpPairingFile::generate(hostname);

    // Dos conexiones: primera inicia el pairing, segunda lo confirma en keychain
    for _ in 0..2 {
        let stream = handle.connect(tunnel_port).await
            .map_err(|e| idevice::IdeviceError::InternalError(e.to_string()))?;
...
async fn place_pairing_auto(dev_udid: &str, app_key: &str) -> Result<(), CliError> {
    ...
    let mut proxy_client = InstallationProxyClient::connect(&provider).await
        .map_err(|e| CliError::Device(e.to_string()))?;
    let apps = proxy_client.get_apps(Some("User"), None).await
        .map_err(|e| CliError::Device(e.to_string()))?;
...
async fn place_file(pairing: Vec<u8>, provider: &impl IdeviceProvider,
    bundle_id: String, path: String) -> Result<(), CliError>
{
...
fn is_ios_below(version: &str, maj_target: u32, min_target: u32) -> bool {
